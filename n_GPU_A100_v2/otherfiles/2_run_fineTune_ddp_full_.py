...
class DDPFullTrainer:
    ...
    def _init_criterion(self):
        if self.args.class_weights:
            ...
            self.pos_weight = w                       # keep it around
        else:
            self.pos_weight = None

        # ALWAYS reduction='none', no pos_weight here
    ...
    def _init_optimizer(self):
        ...
        for name, param in self.model.named_parameters():
            if param.requires_grad:
                if 'module.base' in name:  # More precise identification of base model parameters
    ...
    # def _train_step(self, chunks, labels, mask, spacing):
        ...
        #         pw = self.pos_weight[j:j+1] if (mask_tensor[j] and self.pos_weight is not None) else None
        #         l  = F.binary_cross_entropy_with_logits(logits[0, j:j+1],
        #                                                 target[j:j+1],
        #                                                 pos_weight=pw,
        #                                                 reduction='none')
        #         if not mask_tensor[j]:
        #             l = l * 0.0          # keep graph, zero contribution
        #         loss += l
        #     active = mask_tensor.sum().item()
        #     if active > 0:
        #         loss = loss / active
        ...
        # # metrics (same code as before) …
        # return {
        #     "loss": loss.item()
        #     # lr is added in fit(), not here anymore
        # }
    ...
    def _train_step(self, chunks, labels, mask, spacing):
        ...
        if chunks.size(0) > self.args.max_chunks:
            ...
            end   = start + self.args.max_chunks
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16):
            ...
            loss = 0.0
            for j in range(logits.size(1)):
                pw = self.pos_weight[j:j+1] if (self.pos_weight is not None and mask_tensor[j]) else None
                l  = F.binary_cross_entropy_with_logits(
                        logits[0, j:j+1], target[j:j+1],
        ...
        # ⇨ scale by 1/accum_steps so total grad magnitude is unchanged
        loss = loss / self.args.accum_steps
        self.scaler.scale(loss).backward()

        # ─── running-accuracy metrics (unchanged) ───────────────────────
        with torch.no_grad():
            for j in range(logits.size(1)):
                if mask_tensor[j]:
                    prob = torch.sigmoid(logits[0, j])
                    pred = (prob > 0.5).float()
                    is_corr = (pred == target[j]).item()
                    if is_corr:
                        self.correct_preds[f"task{j}"] += 1
                    self.total_preds[f"task{j}"] += 1
                    # confusion-matrix counters …
                    if   pred == 1 and target[j] == 1: self.tp_preds[f"task{j}"] += 1
                    elif pred == 0 and target[j] == 1: self.fn_preds[f"task{j}"] += 1
                    elif pred == 0 and target[j] == 0: self.tn_preds[f"task{j}"] += 1
                    elif pred == 1 and target[j] == 0: self.fp_preds[f"task{j}"] += 1

        running = {}
        for j in range(len(self.label_cols)):
            tot = self.total_preds[f"task{j}"]
            if tot:
                acc  = self.correct_preds[f"task{j}"] / tot
                tp,fn = self.tp_preds[f"task{j}"], self.fn_preds[f"task{j}"]
                tn,fp = self.tn_preds[f"task{j}"], self.fp_preds[f"task{j}"]
                sens = tp / (tp + fn) if (tp+fn) else float("nan")
                spec = tn / (tn + fp) if (tn+fp) else float("nan")
                running.update({f"acc_task{j}_running":acc,
                                f"sens_task{j}_running":sens,
                                f"spec_task{j}_running":spec})

        return {"loss": loss.item(), **running}   # lr will be added in fit()
    ...
    def evaluate(self, val_loader):
        ...
        if self.args.rank == 0:
            ...
            for i in range(all_preds.shape[1]):
                ...
                true = task_targets.astype(int)
                acc = (pred_labels == true).mean()
                # sensitivity = TP / (TP + FN)
                tp = ((pred_labels == 1) & (true == 1)).sum()
                fn = ((pred_labels == 0) & (true == 1)).sum()
                sens = tp / (tp + fn) if (tp + fn) > 0 else np.nan
                
                # Calculate specificity = TN / (TN + FP)
                tn = ((pred_labels == 0) & (true == 0)).sum()
                fp = ((pred_labels == 1) & (true == 0)).sum()
                spec = tn / (tn + fp) if (tn + fp) > 0 else np.nan
                
                metrics[f'acc_task{i}']   = acc
                metrics[f'sens_task{i}']  = sens
                metrics[f'spec_task{i}']  = spec
    ...
    def fit(self, train_loader, train_sampler, val_loader, epochs, unfreeze_strategy):
        ...
        for epoch in range(epochs):
            ...
            for step, (chunks, labels, mask, spacing) in enumerate(train_loader, 1):
                ...
                update_now = ((self.global_step + 1) % self.args.accum_steps) == 0
                ddp_ctx = self.model.no_sync() if not update_now else nullcontext()
                ...
                if update_now:
                    ...
                    self.scheduler.step()        # one LR hop per real update

                cur_lr = self.scheduler.get_last_lr()[1]
                step_metrics["lr"] = cur_lr
    ...
    def _run_validation(self, val_loader):
        ...
        if self.args.rank == 0:
            ...
            print("Per‐task validation results:")
            for i in range(len(self.label_cols)):
                auc = val_metrics.get(f'auc_task{i}',  float('nan'))
                acc = val_metrics.get(f'acc_task{i}',  float('nan'))
                sens = val_metrics.get(f'sens_task{i}', float('nan'))
                spec = val_metrics.get(f'spec_task{i}', float('nan'))
                print(f"  Task {i}:  AUC {auc:.3f}   Acc {acc:.3f}   Sens {sens:.3f}   Spec {spec:.3f}")
    ...
    def reset_running_metrics(self):
        ...
        self.tp_preds    = {f"task{i}": 0 for i in range(len(self.label_cols))}
        self.fn_preds    = {f"task{i}": 0 for i in range(len(self.label_cols))}
        self.tn_preds    = {f"task{i}": 0 for i in range(len(self.label_cols))}
        self.fp_preds    = {f"task{i}": 0 for i in range(len(self.label_cols))}



def main(args):
    #print(f"Starting main on rank {global_rank}")
    # ------------------------------------------------------------------
    ...
    #print(f"DDP initialized on rank {global_rank}")
    ...
    #print(f"Processor created on rank {global_rank}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="./output_ddp_p16_23_1150_ac2000",
