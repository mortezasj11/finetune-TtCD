    ...
    def _train_step(self, chunks, labels, mask, spacing):
            ...
            for j in range(logits.size(1)):
                if mask_tensor[j]:
                    ...
                    task_loss = F.binary_cross_entropy_with_logits(
                                logits[0, j:j+1],
                                target[j:j+1],
                                pos_weight=pw,    # pw is a 1-element tensor or None
                                reduction='none'
                            )
                    ...
                    # Maybe later I can change logit where mask is False to 0.0. Actually in training .
                    loss += task_loss
        ...
        with torch.no_grad():
            for j in range(logits.size(1)):
                if mask_tensor[j]:
                    ...
                    if pred == 1.0 and target[j] == 1.0:
                        self.tp_preds[f"task{j}"] += 1
                    elif pred == 0.0 and target[j] == 1.0:
                        self.fn_preds[f"task{j}"] += 1
                    elif pred == 0.0 and target[j] == 0.0:
                        self.tn_preds[f"task{j}"] += 1
                    elif pred == 1.0 and target[j] == 0.0:
                        self.fp_preds[f"task{j}"] += 1
        
        # Calculate running accuracies, sensitivity, and specificity
        running_accuracies = {}
        for j in range(len(self.label_cols)):
            ...
            if self.total_preds[task_key] > 0:
                ...
                # Calculate running sensitivity (TP / (TP + FN))
                tp = self.tp_preds[task_key]
                fn = self.fn_preds[task_key]
                sens = tp / (tp + fn) if (tp + fn) > 0 else float('nan')
                running_accuracies[f'sens_task{j}_running'] = sens
                
                # Calculate running specificity (TN / (TN + FP))
                tn = self.tn_preds[task_key]
                fp = self.fp_preds[task_key]
                spec = tn / (tn + fp) if (tn + fp) > 0 else float('nan')
                running_accuracies[f'spec_task{j}_running'] = spec
